export const getCurrentPageProducts = (currentProductsPage, pageSize, productsCountInDB) => {
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const request = await fetch(`http://localhost:5000/products?page=${currentProductsPage}&pageSize=${pageSize}&sendTotalProductsCount=${productsCountInDB}`);
...
export const getProduct = (productID) => {
    return  async (dispatch, getState) => {
        ...
        try {
            const getProductRequest = await fetch(`http://localhost:5000/product/${productID}`);
